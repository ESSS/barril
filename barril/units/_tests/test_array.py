...
def testCategoryParameters(unit_database_len):
    ...
    with mock.patch.object(a2, '_DoValidateValues', autospec=True):
...
def testGetValues():
    ...
    assert type(array.GetValues()) is list
    assert type(array.GetValues('degC')) is list

    array = units.Array('temperature', values=(0, 100), unit='degC')
    assert type(array.GetValues()) is tuple
    assert type(array.GetValues('degC')) is tuple
    assert array[1] == 100
...
def testNumpyConversion():
    ...
    assert type(converted_values) is numpy.ndarray
