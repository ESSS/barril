...
def IsBasicType(value, accept_compound=False, additional=None):
    ...
    """
    if (
        isinstance(value, _ACCEPTED_BASIC_TYPES)
        or value is None
        or (additional and IsInstance(value, additional))
    ):
        return True
...
def IterFlattened(iterable, skip_types=None):
    ...
    for element in iterable:
        element_iter = (
            None
        )  # will be None if element is not iterable, or hold an iterator otherwise
...
def StructMap(obj, func, conditional=lambda x: True):
    ...
    if isinstance(obj, tuple):
        return tuple(StructMap(i, func, conditional) for i in obj)
    if isinstance(obj, list):
        return [StructMap(i, func, conditional) for i in obj]
    if conditional(obj):
        obj = func(obj)
    return obj
