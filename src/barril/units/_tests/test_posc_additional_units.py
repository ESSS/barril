...
def testMassConsumptionEfficiency(db) -> None:
...
def testDensityGeneration(db) -> None:
...
def testVolumePerWtpercent(db) -> None:
...
def testTemperaturePerLengthdegCP30m() -> None:
...
def testAccelerationLinearFeetPerMin2(db) -> None:
...
def testMassConcentration(db) -> None:
...
def testSpecificVolume(db) -> None:
    assert approx(db.Convert("specific volume", "m3/kg", "l/mg", 1.0)) == 0.001


def testViscosityPerPressure(db) -> None:
...
def testSelfInductance(db) -> None:
...
def testConcentrationPerSquareTime(db) -> None:
...
def testFluidGasConcentration() -> None:
    from barril.units import Scalar
